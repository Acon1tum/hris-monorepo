...
const prisma = new client_job_1.PrismaClient();
async function main() {
    ...
    const hrManager = await prisma.jobTitle.create({
        data: {
            ...
            employment_type: client_job_1.EmploymentType.Plantilla,
            salary_grade: 'SG_18',
    ...
    const softwareDev = await prisma.jobTitle.create({
        data: {
            ...
            employment_type: client_job_1.EmploymentType.Contractual,
            salary_grade: 'SG_15',
    ...
    const adminUser = await prisma.user.create({
        data: {
            ...
            role: client_job_1.RoleType.Admin,
        },
    });
    // Create HR user
    ...
    const jobPosting1 = await prisma.jobPosting.create({
        data: {
            ...
            employment_type: client_job_1.EmploymentType.Contractual,
            work_arrangement: client_job_1.WorkArrangement.Hybrid,
            experience_level: client_job_1.ExperienceLevel.Mid_Level,
            education_level: client_job_1.EducationLevel.Bachelor,
            num_vacancies: 2,
            application_deadline: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
            posting_status: client_job_1.PostingStatus.Published,
            job_category: client_job_1.JobCategory.Information_Technology,
    ...
    const jobPosting2 = await prisma.jobPosting.create({
        data: {
            ...
            employment_type: client_job_1.EmploymentType.Plantilla,
            work_arrangement: client_job_1.WorkArrangement.On_Site,
            experience_level: client_job_1.ExperienceLevel.Senior_Level,
            education_level: client_job_1.EducationLevel.Bachelor,
            num_vacancies: 1,
            application_deadline: new Date(Date.now() + 45 * 24 * 60 * 60 * 1000), // 45 days from now
            posting_status: client_job_1.PostingStatus.Published,
            job_category: client_job_1.JobCategory.Human_Resources,
