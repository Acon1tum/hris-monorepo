...
exports.JobPortalManagementController = void 0;
const client_job_1 = require("@prisma/client-job");
const jobDb = new client_job_1.PrismaClient();
function toPrismaEmploymentType(type) {
    const map = {
        Plantilla: 'Plantilla',
...
class JobPortalManagementController {
    static async createJobPosting(req, res) {
        const { position_title, department_id, job_description, qualifications, technical_competencies, salary_range, employment_type, num_vacancies, application_deadline, posting_status, experience_level = 'Mid_Level', job_category = 'Information_Technology' } = req.body;
    ...
    static async getJobPosting(req, res) {
        const { id } = req.params;
    ...
    static async updateJobPosting(req, res) {
        const { id } = req.params;
        const { position_title, department_id, job_description, qualifications, technical_competencies, salary_range, employment_type, num_vacancies, application_deadline, posting_status, experience_level, job_category } = req.body;
        ...
        const updated = await jobDb.jobPosting.update({
            ...
            data: {
                ...
                ...(employment_type ? { employment_type: toPrismaEmploymentType(employment_type) } : {}),
    ...
    static async updateJobPostingStatus(req, res) {
        const { id } = req.params;
        const { status } = req.body;
        const updated = await jobDb.jobPosting.update({ where: { id }, data: { posting_status: status } });
        const withIncludes = await jobDb.jobPosting.findUnique({
            where: { id: updated.id },
...
exports.JobPortalManagementController = JobPortalManagementController;
//# sourceMappingURL=portal-management.controller.js.map