...
export class PersonnelController {
  static async getAllPersonnel(req: Request, res: Response) {
    ...
    const [items, total] = await Promise.all([
      personnelDb.personnel.findMany({
        where,
      ...
      personnelDb.personnel.count({ where })
    ]);
  ...
  static async getPersonnelStats(req: Request, res: Response) {
    const [total, active, byDept] = await Promise.all([
      personnelDb.personnel.count(),
      accounts.user.count({ where: { isActive: true } }),
      personnelDb.personnel.groupBy({ by: ['department_id'], _count: { id: true } })
    ]);
  ...
  static async getDashboardStats(req: Request, res: Response) {
    const [personnelCount, departments, recentHires] = await Promise.all([
      personnelDb.personnel.count(),
      personnelDb.department.count(),
      personnelDb.personnel.findMany({ orderBy: { date_hired: 'desc' }, take: 5, select: { first_name: true, last_name: true, date_hired: true } })
    ]);
  ...
  static async getPersonnelById(req: Request, res: Response) {
    ...
    const person = await personnelDb.personnel.findUnique({ where: { id }, include: { department: true } });
  ...
  static async createPersonnel(req: Request, res: Response) {
    try {
      ...
      if (username && email && password) {
        ...
        const user = await accounts.user.create({
          data: {
            username,
            email,
      ...
      const created = await personnelDb.personnel.create({ 
  ...
  static async updatePersonnel(req: Request, res: Response) {
    const { id } = req.params as any;
    const data = req.body;
    const updated = await personnelDb.personnel.update({ where: { id }, data });
    res.json({ success: true, data: updated });
  }

  static async deletePersonnel(req: Request, res: Response) {
    const { id } = req.params as any;
    await personnelDb.personnel.delete({ where: { id } });
    res.json({ success: true, message: 'Deleted' });
  }

  static async getEmploymentHistory(req: Request, res: Response) {
    const { id } = req.params as any;
    const history = await personnelDb.employmentHistory.findMany({ where: { personnel_id: id }, orderBy: { start_date: 'desc' } });
    res.json({ success: true, data: history });
  }

  static async addEmploymentHistory(req: Request, res: Response) {
    const { id } = req.params as any;
    const created = await personnelDb.employmentHistory.create({ data: { ...req.body, personnel_id: id } });
    res.status(201).json({ success: true, data: created });
  }

  static async getMembershipData(req: Request, res: Response) {
    const { id } = req.params as any;
    const membership = await personnelDb.$queryRawUnsafe<any[]>(`SELECT * FROM membership_data WHERE personnel_id = ${id}`);
    res.json({ success: true, data: membership });
  }

  static async updateMembershipData(req: Request, res: Response) {
    const { id } = req.params as any;
    const updated = await personnelDb.$executeRawUnsafe(`UPDATE membership_data SET data = data || ${JSON.stringify(req.body)} WHERE personnel_id = ${JSON.stringify(id)}`);
    res.json({ success: true, data: updated });
  }

  static async getMeritsViolations(req: Request, res: Response) {
    const { id } = req.params as any;
    const items = await personnelDb.meritViolation.findMany({ where: { personnel_id: id }, orderBy: [{ id: 'desc' }] });
    res.json({ success: true, data: items });
  }

  static async addMeritViolation(req: Request, res: Response) {
    const { id } = req.params as any;
    const created = await personnelDb.meritViolation.create({ data: { ...req.body, personnel_id: id } });
    res.status(201).json({ success: true, data: created });
  }

  static async getAdministrativeCases(req: Request, res: Response) {
    const { id } = req.params as any;
    const items = await personnelDb.administrativeCase.findMany({ where: { personnel_id: id }, orderBy: { created_at: 'desc' } });
    res.json({ success: true, data: items });
  }

  static async addAdministrativeCase(req: Request, res: Response) {
    const { id } = req.params as any;
    const created = await personnelDb.administrativeCase.create({ data: { ...req.body, personnel_id: id } });
    res.status(201).json({ success: true, data: created });
  }

  static async getPersonnelMovements(req: Request, res: Response) {
    const { id } = req.params as any;
    const items = await personnelDb.personnelMovement.findMany({ where: { personnel_id: id }, orderBy: { effective_date: 'desc' } });
    res.json({ success: true, data: items });
  }

  static async addPersonnelMovement(req: Request, res: Response) {
    const { id } = req.params as any;
    const created = await personnelDb.personnelMovement.create({ data: { ...req.body, personnel_id: id } });
    res.status(201).json({ success: true, data: created });
  }

  static async getDashboardEmployees(req: Request, res: Response) {
    const items = await personnelDb.personnel.findMany({ select: { id: true, first_name: true, last_name: true }, orderBy: [{ last_name: 'asc' }] });
    res.json({ success: true, data: items });
  }
  ...
  static async uploadDocuments(req: Request, res: Response) {
    ...
    const created = await personnelDb.employeeDocument.createMany({
  ...
  static async getDocuments(req: Request, res: Response) {
    const { id } = req.params as any;
    const items = await personnelDb.employeeDocument.findMany({ where: { personnelId: id }, orderBy: { createdAt: 'desc' } });
    res.json({ success: true, data: items });
  }
}
