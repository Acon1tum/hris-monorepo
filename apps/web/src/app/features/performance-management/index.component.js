...
exports.PerformanceManagementComponent = void 0;
const core_1 = require("@angular/core");
const common_1 = require("@angular/common");
const forms_1 = require("@angular/forms");
let PerformanceManagementComponent = class PerformanceManagementComponent {
    cdr;
    ...
    getAvgDummy(deptMap, department) {
    ...
    getMostCommonDummy(deptMap, department) {
        if (department && deptMap[department] !== undefined) {
    ...
    getDefaultKpiValue(title, department) {
    ...
    getKpiForDepartment(department, kpi) {
        let emps;
        if (!department) {
            emps = this.employees;
        }
    ...
    toggleKpiChecked(index) {
        const filteredKpi = this.filteredCustomKPIs[index];
        const actualIndex = this.customKPIs.findIndex(kpi => kpi.title === filteredKpi.title &&
    ...
    getSatisfactionTrendsPath(trends) {
        if (!trends || trends.length !== 6)
            return '';
    ...
    getSatisfactionTrendsLine(trends) {
        if (!trends || trends.length !== 6)
            return '';
        const xStep = 472 / 5;
        const yMin = 20, yMax = 130;
    ...
    getDepartmentBarColor(deptName, alpha = 0.5) {
    ...
    fadeInChecklist = true;
    moveDownChecklist = true;
    constructor(cdr) {
        this.cdr = cdr;
    }
    ...
    getKpiDepartmentLabel(kpi) {
        if (!kpi.department || kpi.department === 'All Departments')
            return 'All Departments';
        return kpi.department;
    }
    getKpiRoleLabel(kpi) {
        if (!kpi.role || kpi.role === 'All Role')
            return 'All Role';
        return kpi.role;
    }
...
exports.PerformanceManagementComponent = PerformanceManagementComponent;
exports.PerformanceManagementComponent = PerformanceManagementComponent = __decorate([
    (0, core_1.Component)({
