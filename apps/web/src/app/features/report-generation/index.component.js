...
exports.ReportGenerationComponent = void 0;
const core_1 = require("@angular/core");
const common_1 = require("@angular/common");
const forms_1 = require("@angular/forms");
const template_management_component_1 = require("./template-management/template-management.component");
const audit_trail_component_1 = require("./audit-trail/audit-trail.component");
const scheduled_reports_component_1 = require("./scheduled-reports/scheduled-reports.component");
const sensitive_reports_component_1 = require("./sensitive-reports/sensitive-reports.component");
...
let ReportGenerationComponent = class ReportGenerationComponent {
    ...
    selectedSensitiveReport = null;
    selectedRoleAccess = null;
    ...
    performanceInsightsTimeout = null;
    ...
    calculateDashboardMetrics() {
        ...
        const exportAttempts = this.auditTrails.filter(audit => audit.action === 'exported' || audit.action === 'printed').length;
        const successfulExports = this.auditTrails.filter(audit => (audit.action === 'exported' || audit.action === 'printed') && audit.status === 'success').length;
    ...
    updateMetricsOnReportGeneration(reportType, department) {
    ...
    onCardHover(type, isHovered) {
        this.cardStates[type] = isHovered ? 'hovered' : 'normal';
    }
    navigateToReport(feature) {
    ...
    deleteSensitiveReport(reportId) {
    ...
    onApplyFilters() {
        ...
        if (!this.filters.dateStart)
            this.filterErrors.push('Start date is required.');
        if (!this.filters.dateEnd)
            this.filterErrors.push('End date is required.');
        if (this.filters.dateStart &&
    ...
    createAuditTrailEntry(action, format) {
    ...
    updateReportGenerationMetrics() {
        ...
        if (totalReportsMetric) {
            ...
            const recentReports = this.auditTrails.filter(audit => new Date(audit.generatedAt).getTime() > Date.now() - (24 * 60 * 60 * 1000) // Last 24 hours
            ).length;
    ...
    updateModuleActivities() {
        ...
        const moduleActivity = this.moduleActivities.find(m => m.module.toLowerCase().includes(reportType));
        if (moduleActivity) {
            moduleActivity.activeUsers++;
            moduleActivity.lastActivity = new Date();
            // Update status based on recent activity
            const recentActivity = this.auditTrails.filter(audit => new Date(audit.generatedAt).getTime() > Date.now() - (60 * 60 * 1000) // Last hour
            ).length;
            if (recentActivity > 5) {
    ...
    updateReportTypeDistribution() {
        const reportType = this.filters.reportType;
        const roleDist = this.roleDistribution.find(r => r.role.toLowerCase().includes(reportType));
        if (roleDist) {
            roleDist.count++;
    ...
    getMostUsedExportFormat() {
        ...
        if (Object.keys(formatCounts).length === 0)
            return 'N/A';
        return Object.keys(formatCounts).reduce((a, b) => formatCounts[a] > formatCounts[b] ? a : b).toUpperCase();
    }
    ...
    calculateOverallTrend() {
        const recentActions = this.auditTrails.filter(audit => new Date(audit.generatedAt).getTime() > Date.now() - (7 * 24 * 60 * 60 * 1000) // Last week
        ).length;
...
exports.ReportGenerationComponent = ReportGenerationComponent;
exports.ReportGenerationComponent = ReportGenerationComponent = __decorate([
    (0, core_1.Component)({
        selector: 'app-report-generation',
        standalone: true,
        imports: [common_1.CommonModule, forms_1.FormsModule, template_management_component_1.TemplateManagementComponent, audit_trail_component_1.AuditTrailComponent, scheduled_reports_component_1.ScheduledReportsComponent, sensitive_reports_component_1.SensitiveReportsComponent],
