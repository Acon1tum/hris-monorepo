...
exports.AdminRequestComponent = void 0;
const core_1 = require("@angular/core");
const common_1 = require("@angular/common");
const forms_1 = require("@angular/forms");
let AdminRequestComponent = class AdminRequestComponent {
    requests = [];
    requestTypes = ['Feature Request', 'Bug Report', 'Support Request', 'Leave', 'Overtime', 'Travel', 'Equipment Request', 'Training Request'];
    workflows = [];
    auditLogs = [];
    analyticsData = this.getEmptyAnalytics();
    complianceReports = [];
    showDialog = false;
    isEdit = false;
    dialogTitle = 'New Request';
    requestForm = this.getEmptyRequest();
    editIndex = null;
    // Workflow Management
    showWorkflowDialog = false;
    isWorkflowEdit = false;
    workflowDialogTitle = 'New Workflow';
    workflowForm = this.getEmptyWorkflow();
    editWorkflowIndex = null;
    // Workflow Step Management
    showStepDialog = false;
    isStepEdit = false;
    stepDialogTitle = 'New Step';
    stepForm = this.getEmptyStep();
    editStepIndex = null;
    currentWorkflowId = '';
    ...
    escalationForm = this.getEmptyEscalationRule();
    editEscalationIndex = null;
    // Analytics & Audit
    ...
    loadRequests() {
        ...
        setTimeout(() => {
            ...
            if (data) {
                this.requests = JSON.parse(data).map((r) => ({
                    ...r,
                    date: r.date,
    ...
    loadWorkflows() {
        ...
        if (data) {
            this.workflows = JSON.parse(data).map((w) => ({
                ...w,
    ...
    loadComplianceReports() {
        ...
        if (data) {
            this.complianceReports = JSON.parse(data).map((report) => ({
                ...report,
    ...
    generateSampleComplianceReports() {
        return [
            {
                id: this.generateId(),
    ...
    logAudit(action, entityType, entityId, userName, details, changes) {
        const auditLog = {
            id: this.generateId(),
            action,
    ...
    openDialog(edit = false, index = null) {
        this.isEdit = edit;
    ...
    openWorkflowDialog(edit = false, index = null) {
    ...
    // Workflow Step Management
    openStepDialog(workflowId, edit = false, stepIndex = null) {
    ...
    // Escalation Rule Management
    openEscalationDialog(workflowId, edit = false, escalationIndex = null) {
        this.currentWorkflowId = workflowId;
    ...
    deleteEscalationRule(workflowId, escalationIndex) {
    ...
    filteredRequests() {
        ...
        if (this.filterText) {
            ...
            filtered = filtered.filter(r => r.type.toLowerCase().includes(filter) ||
    ...
    filteredWorkflows() {
        ...
        if (this.workflowFilterText) {
            ...
            filtered = filtered.filter(w => w.name.toLowerCase().includes(filter) ||
    ...
    filteredAuditLogs() {
        ...
        if (this.selectedAuditFilters.dateRange) {
            ...
            filtered = filtered.filter(log => log.timestamp.toDateString() === date.toDateString());
        }
        return filtered;
    }
    // Utility Methods
...
exports.AdminRequestComponent = AdminRequestComponent = __decorate([
    (0, core_1.Component)({
...
//# sourceMappingURL=admin-request.component.js.map