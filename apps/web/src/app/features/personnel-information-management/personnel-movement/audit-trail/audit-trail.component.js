...
let AuditTrailComponent = class AuditTrailComponent {
    ...
    applyFilters() {
        ...
        if (this.searchTerm.trim()) {
            ...
            filtered = filtered.filter(audit => audit.user.toLowerCase().includes(search) ||
        ...
        if (this.selectedDateRange) {
            ...
            switch (this.selectedDateRange) {
                case 'today':
                    filtered = filtered.filter(audit => audit.timestamp >= today);
                    break;
                case 'week':
                    const weekAgo = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);
                    filtered = filtered.filter(audit => audit.timestamp >= weekAgo);
                    break;
                case 'month':
                    const monthAgo = new Date(today.getFullYear(), today.getMonth() - 1, today.getDate());
                    filtered = filtered.filter(audit => audit.timestamp >= monthAgo);
                    break;
                case 'quarter':
                    const quarterAgo = new Date(today.getFullYear(), today.getMonth() - 3, today.getDate());
                    filtered = filtered.filter(audit => audit.timestamp >= quarterAgo);
                    break;
                case 'year':
                    const yearAgo = new Date(today.getFullYear() - 1, today.getMonth(), today.getDate());
                    filtered = filtered.filter(audit => audit.timestamp >= yearAgo);
                    break;
            }
        }
